...
export class ListenerProcess {
  ...
  genreFinderLogic(movie, handlerName, selected) {
    var selectedLogic = movie[handlerName].map(finded => {
  ...
  filterListenerHandler(element, handlerName) {
    element.addEventListener("click", () => {
      ...
      var selectedValues = this.handleFilter(handlerName);
      selectedValues.forEach(selected => {
        this.movieSelf.currentData.filter(movie => {
  ...
  handleFilter(handlerName) {
    var selectedFilter = Array.prototype.slice.call(document.querySelectorAll("input[name='".concat(handlerName, "']:checked")));
    var selected = selectedFilter.map(selected => selected.value);
    return selected;
  }
  ...
  handleSearch() {
    this.movieSelf.$searchForm.addEventListener("submit", event => {
      ...
      resetBg(this.movieSelf.$tbodyEl, "tr");
      var searchValue = this.movieSelf.$searchInput.value;
  ...
  movieLineListenerHandler(currentData) {
    ...
    currentData.forEach(movie => {
      document.querySelector("tr[data-id=\"".concat(movie.id, "\"]")).addEventListener("click", function (event) {
  ...
  clearFilter() {
    Array.prototype.slice.call(document.querySelectorAll("input[class='form-check-input']:checked")).forEach(selected => selected.checked = false);
    this.movieSelf.$tbodyEl.innerHTML = "";
    this.movieSelf.currentData.forEach(movie => {
      this.movieSelf.$tbodyEl.innerHTML += this.movieSelf.createMovieLine(movie);
    });
